...
**Window lifecycle — close hides, app keeps running.** The red traffic-light close *hides* the main window instead of closing it (`windowShouldClose` returns `false` after `hideMainWindow()`, which flushes pending saves first); `applicationShouldTerminateAfterLastWindowClosed` returns `false`, so quitting is only via ⌘Q or the menu-bar extra. A Dock click reopens through `applicationShouldHandleReopen` — which can fire *before* `applicationDidFinishLaunching` has created the window, so it defers via `pendingShowMainWindow`. macOS **window state restoration is opted out entirely**: `NSQuitAlwaysKeepsWindows=false` is registered in `applicationWillFinishLaunching`, `shouldSave/shouldRestoreApplicationState` return `false`, the window is `isRestorable = false`, and any stale `.savedState` folder is deleted at launch (`discardStaleSavedApplicationState`). The windows are created in code; letting macOS save/restore them across a system restart fought that setup and crashed relaunch ("unexpectedly quit while reopening windows"). Don't remove any one of these — a leftover snapshot from an older build alone can retrigger the dialog.
...
`Note` is a `Codable` struct. Rich text is stored as RTF `Data`; titles are stored separately on the struct. It also carries `deletedAt` (non-nil ⇒ trashed), `isPinned`, `tags: [String]`, `kind: NoteKind` (`.richText` / `.apiManager` — see *API Key Manager pages*), `providers` (the API page payload; `nil` for notes), and `titleIsManual`. A hand-written `init(from:)` decodes missing keys to safe defaults, so `notes.json` written by older versions (which lacked these fields) still loads (`kind` defaults to `.richText`; `APIProvider` likewise defaults a missing `baseURL` — without that, one legacy provider bricks the whole store).

**Auto-titles.** Unless `titleIsManual` is set, a rich-text note's title is derived from its first non-empty body line — `NoteStore.autoTitle(from:)` strips list/heading/quote markers and keeps at most 3 words — and re-synced live while typing via `syncAutoTitleIfNeeded(for:persist:)` (called from the keystroke path with `persist: false` so the tab/sidebar update without a disk write; the debounced flush persists it). `titleIsManual` is set when the user renames a tab (`markTitleAsManual`), on import (`addNote` titles come from filenames), and on API pages — those titles are never overwritten.
...
### API Key Manager pages

A second page kind alongside rich-text notes: `Note.kind == .apiManager` carries a structured `providers: [APIProvider]` payload (provider `name` + `baseURL` + `keys: [APIKeyEntry]`, each key a `label`/`value`/`createdAt`) instead of `rtfData`. Created from the sidebar via `NoteStore.newAPIPage()` — the page arrives **auto-pinned** with `titleIsManual` set. `ContentView` renders `APIManagerView` for these tabs instead of `NoteEditorView` (the RTF editor assumes an `rtfData` body + shared `NSTextStorage` the API page doesn't have).

- **All provider/key mutation funnels through `NoteStore.mutateProviders(_:_:)`** — it kind-guards (silently no-ops on a rich-text note) and bumps `updatedAt` + persists on every change. Route any new mutation through it. **Provider reorder** via `moveProvider` (`toIndex` for absolute position, `toInsertionSlot` for drag-between drops — slot 0 is the top). `APIManagerView` shows insertion lines while dragging plus ▲/▼ and a context menu (Move to Top / Bottom) on provider cards only — keys inside a provider are not reordered; reorder is disabled while search is filtering.
- **Secrets never leak into search or snippets.** `plainText(for:)` returns `""` for API pages, and `filteredNotes` matches them via `apiSearchText(for:)` — provider names + key labels only, never key values. Keys render masked (`MenuBarAPIEntry.mask`: bullets + last 4 chars); the UI copies values via `NoteStore.copyToPasteboard`.
- Export is refused for API pages, and they're excluded from the RTF-only paths generally. Tag/pin/trash behave like any note.

Covered by `Tests/NoteFlowTests/APIManagerTests.swift` (CRUD round-trips, legacy JSON decode, search/secret exclusion, export refusal).

### Menu-bar extra

`MenuBarController.shared.install()` (called from `applicationDidFinishLaunching`) puts an `NSStatusItem` with the app logo in the menu bar: a **Pinned** section, an **API** submenu (grouped page → provider, with copy-only "Copy key · …" / "Copy URL · …" items — key titles are masked), **Show NoteFlow**, and **Quit**. The menu is rebuilt on every open (`menuWillOpen`) plus on `.menuBarPinsChanged`, so it always reflects the live `NoteStore`. **The API submenu is a small fixed-height scrollable list** (`MenuBarAPIListView`, capped at `maxVisibleRows`) — NSMenu itself has no height cap and would stretch toward full screen for long key lists, so the rows live in a custom-view scroll view instead. Row grouping is built by `MenuBarController.apiMenuRows` (page → provider → copy actions). `populateAPIMenu` is internal and takes `pageOrder` explicitly (rather than reading `NoteStore.shared` inside) so tests can pin the layout. `MenuBarPinStore` (in `Models/MenuBarPinStore.swift`) decides what appears under Pinned — three `UserDefaults`-persisted UUID sets (pinned keys, providers, whole pages; a key counts as pinned if any of the three matches). The entry rows are the value types `MenuBarAPIEntry` / `MenuBarProviderEntry` built by `NoteStore.allMenuBarAPIEntries()` / `allMenuBarProviderEntries()`. Covered by `Tests/NoteFlowTests/MenuBarTests.swift`.
...
NoteFlowApp (@main)
└── AppDelegate
    ├── MenuBarController (NSStatusItem)  # menu-bar extra: pinned entries + API copy menu
    ├── window (NSWindow)
    ...
    │               │                     # (.apiManager tabs render APIManagerView here instead)
